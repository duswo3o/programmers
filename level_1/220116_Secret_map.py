...
def solution1(n,arr1,arr2):
    arr1 = [int(bin(i)[2:]) for i in arr1]
    arr2 = [int(bin(i)[2:]) for i in arr2]
    
    s_map = [str(arr1[i]+arr2[i]) for i in range(len(arr1))]

    n_map = []
    for i in range(len(s_map)):
        if len(s_map[i])!=n:
            n_map.append('0'*(n-len(s_map[i]))+s_map[i])
        else:
            n_map.append(s_map[i])
